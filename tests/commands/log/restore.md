# `gage log restore` command
    ...
    >>> run("gage log restore --help")
    Restore deleted logs
    ⤶
    Usage: gage log restore [OPTIONS] [SPEC]...
    ⤶
    Arguments:
      [SPEC]...
              Log(s) to restore
...
Delete the run.
    ...
    >>> run("gage log delete --all -y", cols=75)  # +parse +table
    ┌  Delete logs
    │
    ├  ╭────────┬──────┬──────┬─────────┬───────────┬─────────┬─────────╮
    │  │ Id     │ Task │ Type │ Status  │ Model     │ Dataset │ Started │
...
Restore the run using the table index.
    ...
    >>> run("gage log restore 1 -y", cols=72)  # +parse +table
    ┌  Restore logs
    │
    ├  ╭────────┬──────┬──────┬─────────┬───────────┬─────────┬─────────╮
    │  │ Id     │ Task │ Type │ Status  │ Model     │ Dataset │ Started │
...
Try to restore the log.
    ...
    >>> run(f"gage log restore {id} -y", cols=72)  # +parse +table
    ┌  Restore logs
    │
    ├  ╭────────┬──────┬──────┬─────────┬───────────┬─────────┬─────────╮
    │  │ Id     │ Task │ Type │ Status  │ Model     │ Dataset │ Started │
