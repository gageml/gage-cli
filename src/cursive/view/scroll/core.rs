...
use cursive::{
    ...
    event::{AnyCb, Event},
    style::Style,
    view::{Selector, SizeCache, ViewNotFound},
};

use crate::cursive::view::scroll::ScrollStrategy;
...
macro_rules! impl_scroller {
    ($class:ident :: $core:ident) => {
        impl $crate::cursive::view::scroll::Scroller for $class {
            fn get_scroller_mut(
                &mut self,
            ) -> &mut $crate::cursive::view::scroll::Core {
                &mut self.$core
            }
            fn get_scroller(&self) -> &$crate::cursive::view::scroll::Core {
                &self.$core
            }
        }
    };
    ($class:ident < $($args:tt),* > :: $core:ident) => {
        impl <$( $args ),* > $crate::cursive::view::scroll::Scroller for $class<$($args),*> {
