...
impl<V: 'static> ScrollView<V> {
    ...
    where
        F: FnMut(&mut cursive::Cursive, Rect) + 'static + Send + Sync,
    {
...
#[cursive::blueprint(ScrollView::new(view))]
struct Blueprint {
    view: cursive::views::BoxedView,
...
//         - scroll: true
// ```
cursive::manual_blueprint!(with scroll, |config, context| {
    use cursive::builder::{Config, Error};
