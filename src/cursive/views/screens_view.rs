...
impl ScreensView<cursive::views::StackView> {
    /// Draws the background.
    ///
    /// This is mostly used internally by cursive. You probably just want
    /// `View::draw`.
    #[allow(dead_code)]
    pub fn draw_bg(&self, printer: &cursive::Printer) {
    ...
    /// `View::draw`.
    #[allow(dead_code)]
    pub fn draw_fg(&self, printer: &cursive::Printer) {
        if let Some(screen) = self.screen() {
...
impl<V> cursive::view::ViewWrapper for ScreensView<V>
where
    V: View,
{
    type V = V;
