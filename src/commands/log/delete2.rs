...
pub fn main(args: Args, config: &Config) -> Result<()> {
    ...
    // Log op dialog
    let prompt =
        (!args.yes).then_some(|logs: &Vec<&LogInfo>| delete_prompt(logs.len(), args.permanent));
...
fn delete_prompt(count: usize, permanent: bool) -> (String, bool) {
    (
        format!(
            "You are about to{} delete {} logs.{} Continue?",
